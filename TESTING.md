# Руководство по ручному тестированию User Context интеграции
...
2. **Для каждого пункта чек-листа:**
   - Выполните действия из "Шаги тестирования"
   - Проверьте ожидаемый результат
   - Отметьте статус: ✅ / ❌ / ⚠️
   - Опишите проблему в "Комментариях" (если есть)
...
**Проверка промпта (после решения проблемы):**
```bash
...
**Шаги тестирования:**
1. Откройте Telegram
2. Найдите бота `@your_problem_solver_bot`
...
3. Используйте один из примеров выше или свою проблему
...
**Комментарии:**
```
(приведите примеры вопросов, которые задал бот)
```
...
**Комментарии:**
```
(скопируйте решение целиком или укажите, что учтено/не учтено)
```
...
**Шаги тестирования:**
1. После генерации решения выполните:
```bash
grep -A 50 "КОНТЕКСТ ПОЛЬЗОВАТЕЛЯ" bot.log | tail -60
```
...
1. Сохраните этот файл с заполненными комментариями
