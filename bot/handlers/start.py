...
async def handle_unhandled_message(message: Message):
    """Handle random messages outside of workflow"""
    import random

    responses = [
        # Юморные
        "Эм, я вижу ты пишешь, но я же бот — мне нужны кнопки! 🤖 Жми '🚀 Решить проблему' внизу 👇",
        "Стоп-стоп, я не Siri 😅 Используй меню внизу, там все кнопки для навигации! 👇",
        "Кажется, ты пытаешься поговорить со мной по душам, но я пока умею только через кнопки 😄 Жми '🚀 Решить проблему' 👇",

        # Нейтральные
        "Чтобы начать работу, используй кнопки в меню внизу 👇",
        "Для навигации по боту используй меню с кнопками внизу экрана 👇",
        "Управление ботом осуществляется через кнопки меню внизу 👇",

        # Дружелюбные
        "Привет! Для работы со мной используй кнопки меню внизу — там все команды 👇",
        "Я работаю через кнопки в меню! Нажми '🚀 Решить проблему', чтобы начать 👇",
        "Спасибо за сообщение! Но лучше используй меню внизу — так удобнее 😊 👇",
        "Вижу ты активен! Чтобы я понял что делать, используй кнопки меню 👇"
    ]

    await message.answer(random.choice(responses), reply_markup=get_main_menu_keyboard())
